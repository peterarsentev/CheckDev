...
public class ProfileService {
    ...
    @Autowired
    public ProfileService(final ProfileRepository profileRepository, final Messenger msg) {
    ...
    public Optional<Profile> reg(Profile profile) {
        ...
        try {
            if (profile.isPrivacy()) {
                ...
                result = Optional.of(this.profileRepository.save(profile));
    ...
    public List<Profile> getIn(List<String> keys) {
        return this.profileRepository.findByKeyIn(keys);
    }

    @Transactional
    public boolean activated(String key) {
        Profile profile = this.profileRepository.findByKey(key);
    ...
    public Optional<Profile> forgot(Profile profile) {
        final Optional<Profile> result;
        Profile find = this.profileRepository.findByEmail(profile.getEmail());
        if (find == null) {
            result = Optional.empty();
        } else {
            String password = RandomStringUtils.randomAlphabetic(8);
            find.setPassword(this.encoding.encode(password));
            this.profileRepository.save(find);
            Map<String, Object> keys = new HashMap<>();
    ...
    public List<Profile> findAll(Pageable pageable) {
        return this.profileRepository.findAll(pageable).getContent();
    }

    public Long total() {
        return this.profileRepository.total();
    }
    ...
    public void saveRole(Profile profile) {
        Profile load = this.profileRepository.findById(profile.getId()).get();
        ...
        this.profileRepository.save(load);
    }

    public Profile findByKey(String key) {
        return this.profileRepository.findByKey(key);
    }

    public List<Profile> findBySearch(String search, PageRequest email) {
        return this.profileRepository.findByEmailContainingOrUsernameContaining(search, search, email).getContent();
    }

    public List<Profile> findByShow(boolean show, int limit) {
        return this.profileRepository.findByShow(show, PageRequest.of(0, limit));
    }

    public List<Profile> findByShow(boolean show) {
        return this.profileRepository.findByShow(show);
    }

    public List<Profile> findByShow(boolean show, Pageable pageable) {
        return this.profileRepository.findByShow(show, pageable);
    }

    public Long showed() {
        return this.profileRepository.showed();
    }
    ...
    public String update(String email, MultipartFile file, Profile profile) {
        String result = "ok";
        Profile profileDb = profileRepository.findPerson(email);
        Photo photo;
